...
- **단위 테스트(Unit test)**
- 단위 테스트에서 테스트 대상 단위의 크기는 엄격하게 정해져 있지 않다고 한다. 하지만, 일반적으로 클래스 또는 메소드 수준으로 정해진다.
- 테스트 대상 단위의 크기를 작게 설정해서 단위 테스트를 최대한 간단하고 디버깅하기 쉽게 작성해야한다고 한다.
- **통합 테스트(Integration test)**
- 통합 테스트는 이름에서 의미하는 바와 같이 각각의 시스템들이 서로 어떻게 상호작용하고 제대로 작동하는지 테스트하는 것을 의미한다.
- 통합 테스트는 단위 테스트와 비슷한데, 큰 차이점이 하나 있다. 단위 테스트는 다른 컴포넌트들과 독립적인 반면 통합 테스트는 그렇지 않다.
- 예를 들자면 단위 테스트에서 데이터베이스에 접근하는 코드는 실제 데이터 베이스와 통신하는 것은 아니지만, 통합 테스트는 실제 통신해야 한다.
